window.kelimeci = function() {
    const NUM_LETTERS = 6;
    const NUMBER_OF_GUESSES = 6;
    const UI_BOARD_TAG_NAME = "game-board";
    const DELAY_PER_LETTER_IN_MS = 250
    ...
    this.trie = function() {
        ...
        words.forEach((word, index, arr) => {
            let current = trie;
            Array.from(word).forEach((ch, chIndex, wordArr) => {
                if (ch in current) {
                } else {
                    current[ch] = {};
                }
    ...
    /* If this was real game, we should have the same selected item for a day
    */
    this.selected = function() {
        ...
        let word = getOne();
        result = {};
        Array.from(word).forEach((ch, ind, arr) => {
            if (ch in result) {
                result[ch].push(ind);
            } else {
                result[ch] = [ind];
    ...
    function guess(word) {
        ...
        letters.forEach((ch, ind, arr) => {
            if (ch in cloned && cloned[ch].includes(ind)) {
                result[ind] = LetterMatchType.Correct;
                cloned[ch] = cloned[ch].filter(function(value, index, arr){ 
                    return value != ind;
                });
                if (cloned[ch].length == 0) {
                    delete cloned[ch];
                }
            }
        });
        letters.forEach((ch, ind, arr) => {
            if (result[ind] == LetterMatchType.Correct) return;

            if (!(ch in cloned)) {
                result[ind] = LetterMatchType.NotExist;
            } else {
                result[ind] = LetterMatchType.Exist;
                if (cloned[ch].length == 1) {
                    delete cloned[ch];
                } else {
                    cloned[ch].shift();
                }
        ...
        return {
            resultType: GuessResult.Existing,
            matchArray: result,
        }
    }
    ...
    function checkGuess (doc) {
        ...
        for (let i=0; i < NUM_LETTERS; i++) {
            let letterColor = "";
            let letter = this.session.currentGuess[i];
            let box = row.children[i];
            switch(guessResult.matchArray[i]) {
                case LetterMatchType.NotExist:
                    letterColor = Color.GREY;
                    break;
                case LetterMatchType.Correct:
                    letterColor =  Color.GREEN;
                    break;
                case LetterMatchType.Exist:
    ...
    return {
        handleEvent: (e, doc) => {
            ...
            if (pressedKey === "Backspace" && this.session.nextLetter !== 0) {
                deleteLetter(doc);
                return;
            }
            ...
            let found = pressedKey.match(/[a-z,ü,ö,ğ,ş,ç,İ]/gi)
        ...
        init: function(doc) {
            ...
            //match clicks to keyboard event
            doc.getElementById("keyboard-cont").addEventListener("click", (e) => {
